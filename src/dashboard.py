...
def _make_market_panel(state: dict) -> Panel:
    ...
    if not window:
        content = Text("  Scanning for active sol-updown-5m window...\n", style=WARN_COLOR)
...
def _make_price_panel(state: dict) -> Panel:
    """ETH current price (Binance) and gap vs price-to-beat."""
    eth = state.get("eth_price")
    window = state.get("window")
    ...
    if eth is not None and eth > 0:
        table.add_row("ETH Price", f"[bold bright_white]${eth:,.2f}[/]")
    else:
        table.add_row("ETH Price", "[dim]Loading...[/]")

    if eth and window and window.price_to_beat:
        gap = eth - window.price_to_beat
        gap_abs = abs(gap)
    ...
    return Panel(table, title="[bold]⟠ ETH Price (Binance)[/]", border_style=BORDER, height=7)
