...
def approve_usdc():
    ...
    if not wallets:
        log.error("No wallets found in POLY_WALLETS!")
        return
    ...
    for idx, wallet_config in enumerate(wallets, 1):
        priv = wallet_config["private_key"]
        acct = w3.eth.account.from_key(priv)
        log.info("=== Wallet %d/%d: %s ===", idx, len(wallets), acct.address)
